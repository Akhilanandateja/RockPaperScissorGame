...
app = Flask(__name__)
app.secret_key = os.getenv('SECRET_KEY', 'your-secret-key-here')  # Fallback for local testing
...
class GameHistory(db.Model):
    ...
    rounds = db.Column(db.String(20), nullable=False)
    ...
    if request.method == 'POST':
        try:
            ...
            if rounds == 'infinity':
                ...
                session['max_rounds'] = int(rounds) if rounds.isdigit() and int(rounds) > 0 else 5
...
def game():
    ...
    if request.method == 'POST':
        if 'quit' in request.form:
            ...
            result = f"Game Over! Final Scores: You: {player_score}, Computer: {computer_score} - {winner} wins!"
            if user or session.get('is_guest'):
                game_entry = GameHistory(
                    user_id=user.id if user else None,
                    player_score=player_score,
                    computer_score=computer_score,
                    rounds='infinity' if max_rounds is None else str(max_rounds),
                    result=winner
                )
    ...
    return render_template('game.html', username=username, player_score=player_score,
                           ...
                           max_rounds=max_rounds, result=result, player_choice=player_choice,
                           computer_choice=computer_choice, game_over=game_over,
                           win_percentage=round(win_percentage, 2), total_games=total_games,
...
def profile():
    ...
    return render_template('profile.html', username=username, player_wins=player_wins,
                           ...
                           computer_scores=computer_scores)
